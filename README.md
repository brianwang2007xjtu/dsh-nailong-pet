...
# 或直接引用 GitHub 仓库
dsh plugin --profile web add github:yourname/dsh-nailong-pet
```

### 2. 挂载到宿主组合

在 web profile 的 `cordis.patch.yml` 里加一行：

```yaml
- id: nailong-pet
  name: dsh-nailong-pet
  config:
...
`id` 对应 `index.js` 里导出的插件名（`export const name = "nailong-pet"`），`name` 是 npm 包名 `dsh-nailong-pet`。

### 3. 重启 / 刷新

重启 web 进程（或触发组合重载），然后刷新浏览器页面，右下角就会出现奶龙。
